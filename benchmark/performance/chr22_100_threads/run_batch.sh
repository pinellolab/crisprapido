...
RUN_ROOT="${RUN_ROOT:-${RAW_DIR}/${RUN_ID:-manual_threads_${THREADS:-1}_$(date -u +%Y%m%dT%H%M%SZ)}}"
BATCH_ID="${BATCH_ID:-${1:-}}"
MODE="${MODE:-${2:-baseline}}"
PHASE="${PHASE:-timing}"
ITERATION="${ITERATION:-${3:-measured_1}}"
THREADS="${THREADS:-${SLURM_CPUS_PER_TASK:-1}}"
FORCE="${FORCE:-0}"

if [[ -z "${BATCH_ID}" ]]; then
  echo "Usage: BATCH_ID=batch_001 MODE=baseline THREADS=2 $0" >&2
  exit 2
fi
...
if [[ -n "${SLURM_CPUS_PER_TASK:-}" && "${SLURM_CPUS_PER_TASK}" != "${THREADS}" ]]; then
...
GUIDE_TSV="${GUIDE_TSV:-${REPO_ROOT}/benchmark/performance/chr22_100_guides/chr22_guides.tsv}"
...
BATCH_DIR="${RUN_ROOT}/threads_${THREADS}/batches/${PHASE}/${MODE}/${ITERATION}/${BATCH_ID}"
...
rm -rf "${BATCH_DIR}"
mkdir -p "${BATCH_DIR}/guides" "${RUN_ROOT}/metadata"

[[ -x "${CRISPRAPIDO_BIN}" ]] || { echo "Missing executable: ${CRISPRAPIDO_BIN}" >&2; exit 1; }
[[ -r "${REFERENCE}" ]] || { echo "Missing reference: ${REFERENCE}" >&2; exit 1; }
[[ -r "${GUIDE_TSV}" && -r "${BATCH_TSV}" ]] || { echo "Missing guide or batch TSV" >&2; exit 1; }
...
[[ -s "${BATCH_DIR}/guide_ids.txt" ]] || { echo "Unknown batch ID: ${BATCH_ID}" >&2; exit 1; }
...
printf 'guide_id\tmode\tphase\titeration\tthreads\texit_status\twall_seconds\tuser_seconds\tsystem_seconds\tpeak_rss_kib\tpaf_records\tstdout_sha256\tstderr_sha256\n' > "${BATCH_DIR}/guide_results.tsv"

while IFS= read -r guide_id; do
  guide_seq="$(awk -F'\t' -v id="${guide_id}" 'NR>1 && $1==id {print $2}' "${GUIDE_TSV}")"
  [[ -n "${guide_seq}" ]] || { echo "Missing guide sequence: ${guide_id}" >&2; exit 1; }
  ...
  records="$(wc -l < "${paf}")"
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "${guide_id}" "${MODE}" "${PHASE}" "${ITERATION}" "${THREADS}" "${status}" "${wall}" "${user}" "${sys}" "${rss}" "${records}" "${out_sha}" "${err_sha}" >> "${BATCH_DIR}/guide_results.tsv"
  if [[ "${status}" != "0" ]]; then
