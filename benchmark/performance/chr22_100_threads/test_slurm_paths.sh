...
(
  cd /tmp
  DRY_RUN=1 RUN_ID_PREFIX=path_test THREAD_SET="2" "${PACKAGE_DIR}/submit_plan.sh" > "${OUT}/submit_plan.txt"
)
...
for script in "${PACKAGE_DIR}"/*.sbatch; do
  ...
  (
    cd /tmp
    SLURM_SUBMIT_DIR="${REPO_ROOT}" SLURM_ARRAY_TASK_ID=1 SLURM_CPUS_PER_TASK=2 THREADS=2 RUN_ID=path_debug PATH_DEBUG=1 bash "${script}" > "${OUT}/$(basename "${script}").paths"
  )
  ...
  grep -q '^THREADS=2$' "${OUT}/$(basename "${script}").paths"
done
echo "thread path dry-run passed for ${PACKAGE_DIR}"
