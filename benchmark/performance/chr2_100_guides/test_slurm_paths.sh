...
(
  cd /tmp
  PACKAGE_DIR="${PACKAGE_DIR}" REPO_ROOT="${REPO_ROOT}" BATCH_ID=batch_001 MODE=columba PHASE=path_smoke ITERATION=pilot FORCE=1 RUN_ROOT="${OUT}/run_root" \
    env -u RUSTFLAGS -u LIBRARY_PATH -u LD_LIBRARY_PATH "${PACKAGE_DIR}/run_batch.sh"
)
if [[ ! -e "${OUT}/run_root/batches/path_smoke/columba/pilot/batch_001/SUCCESS" ]]; then
  echo "missing smoke SUCCESS marker" >&2
  exit 1
fi
find "${OUT}" -maxdepth 4 -print | grep -q '/var/spool' && { echo "spool path created" >&2; exit 1; }
