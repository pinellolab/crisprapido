...
pub fn init_score_matrices(mismatch_path: &str, pam_path: &str) -> Result<(), String> {
    INIT.call_once(|| {
        ...
        let pam_matrix =
            parse_scoring_matrix(pam_path).map_err(|e| format!("Failed to load PAM scores: {}", e));
...
pub fn calculate_cfd(spacer: &str, protospacer: &str, pam: &str) -> Result<f64, String> {
    ...
    let spacer_20bp = if spacer.len() >= 20 {
        if spacer.contains('-') {
            ...
            let truncated = if spacer.len() > 20 {
                &spacer[0..20]
            } else {
                spacer
            };
        ...
        return Err(format!(
            "Spacer too short: {} bp, expected at least 20 bp",
            spacer.len()
        ));
    };
    ...
    let protospacer_20bp = if protospacer.len() >= 20 {
        if protospacer.contains('-') {
            let truncated = if protospacer.len() > 20 {
                &protospacer[0..20]
            } else {
                protospacer
            };
        ...
        return Err(format!(
            "Protospacer too short: {} bp, expected at least 20 bp",
            protospacer.len()
        ));
    };
    ...
    let pam_processed = match pam_clean.len() {
        ...
        _ => {
            return Err(format!(
                "PAM must be 2 or 3 nucleotides, got {} bp",
                pam.len()
            ))
        }
    };
    ...
    if spacer_list.len() != 20 || protospacer_list.len() != 20 {
        return Err(format!(
            "Processed sequences must be 20bp: spacer={}, protospacer={}",
            spacer_list.len(),
            protospacer_list.len()
        ));
    }
...
fn prepare_aligned_sequences(guide: &[u8], target: &[u8], cigar: &str) -> (String, String) {
    ...
    if cigar.is_empty() {
        ...
        let spacer_20 = if guide_str.len() >= 20 {
            &guide_str[0..20]
        } else {
            &guide_str
        };
        let target_20 = if target_str.len() >= 20 {
            &target_str[0..20]
        } else {
            &target_str
        };
...
fn parse_scoring_matrix(file_path: &str) -> Result<HashMap<String, f64>, String> {
    // Open file
    let file = File::open(file_path).map_err(|e| format!("Cannot open {}: {}", file_path, e))?;
...
mod cfd_unit_tests {
    ...
    fn test_position_1_gap_no_penalty() {
        ...
        assert!(
            (score - 1.0).abs() < 1e-9,
            "Gap at position 1 should have no penalty, got {}",
            score
        );
    ...
    fn test_gaps_at_other_positions_have_penalties() {
        ...
        assert!(
            (score - 0.0).abs() < 1e-9,
            "Gap at position 7 should be 0.0, got {}",
            score
        );
        ...
        assert!(
            score < 1.0,
            "Gap at position 2 should reduce score, got {}",
            score
        );
    }
    ...
    fn test_case_insensitivity() {
        ...
        assert!(
            (score_upper - score_mixed).abs() < 1e-9,
            "Case should not matter: upper={}, mixed={}",
            score_upper,
            score_mixed
        );
    }
    ...
    fn test_t_u_equivalence() {
        ...
        assert!(
            (score_t - score_u).abs() < 1e-9,
            "T and U should be equivalent: T={}, U={}",
            score_t,
            score_u
        );
    }
    ...
    fn test_scores_in_valid_range() {
        ...
        for pos in [1, 5, 10, 15, 20] {
            for &new_base in &bases {
                ...
                assert!(
                    score >= 0.0 && score <= 1.0,
                    "Score out of range [0,1]: {} for pos {} -> {}",
                    score,
                    pos,
                    new_base
                );
    ...
    fn test_prepare_aligned_sequences() {
        ...
        assert!(
            proto.contains('-'),
            "Insertion should create gap in protospacer"
        );
